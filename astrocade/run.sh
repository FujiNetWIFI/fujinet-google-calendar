#!/usr/bin/env bash
# run.sh -- run the built client in MAME against a live fujinet-pc.
#
...
# keypad 1 to start GCAL. The fujinet-pc needs the GCAL: adapter (firmware
# master), a POSIX [General] timezone, [Device] enable_apetime=1, and Google
# authorised in the web UI with calendar.events in the grant's scope.
...
exec "$MAME_DIR/mame" astrocde \
    ...
    -cartslot fujinet -cart "$PWD/build/gcal.bin" \
    -window -nomax \
    "$@"
