...
' comes up under --fujinet even though it works on real hardware.
    ...
    ' place account for the whole $8000-$9BFF window.
...
' caps at three digits and suppresses leading zeros -- neither of which a
' fixed-width date field can tolerate. 16-bit input, so years are unbounded
' for our purposes.
' ---------------------------------------------------------------------------
DIM #pn_val4
fn_putnum4: PROCEDURE
    POKE (FN_TX + #fn_txlen + 0), (#pn_val4 / 1000) % 10 + 48
    POKE (FN_TX + #fn_txlen + 1), (#pn_val4 / 100) % 10 + 48
    POKE (FN_TX + #fn_txlen + 2), (#pn_val4 / 10) % 10 + 48
    POKE (FN_TX + #fn_txlen + 3), #pn_val4 % 10 + 48
    #fn_txlen = #fn_txlen + 4
END

' ---------------------------------------------------------------------------
' fn_putnum3: append #pn_val4 as up to three decimal digits, no leading zeros.
' fn_putnum above takes an 8-bit input, which cannot hold a GCAL event number
' -- the adapter caps a window at 300 events, so /N runs to three digits.
' ---------------------------------------------------------------------------
fn_putnum3: PROCEDURE
    pn_started = 0
    pn_h = (#pn_val4 / 100) % 10
    IF pn_h > 0 THEN
        POKE (FN_TX + #fn_txlen), pn_h + 48 : #fn_txlen = #fn_txlen + 1
    ...
    IF pn_t > 0 OR pn_started THEN
        POKE (FN_TX + #fn_txlen), pn_t + 48 : #fn_txlen = #fn_txlen + 1
    END IF
    POKE (FN_TX + #fn_txlen), (#pn_val4 % 10) + 48 : #fn_txlen = #fn_txlen + 1
END

' ---------------------------------------------------------------------------
' fn_putnum2: append pn_val as exactly two zero-padded decimal digits, for
' the month and day fields of the same date.
' ---------------------------------------------------------------------------
fn_putnum2: PROCEDURE
    POKE (FN_TX + #fn_txlen + 0), (pn_val / 10) % 10 + 48
