...
{
    /* Slot, then the three 0-3 guns. A table and a loop rather than thirteen
       calls: the calls came to 262 bytes on this compiler and this is a third
       of that, which on a machine with a few hundred to spare is worth the
       indirection. */
    static const unsigned char pal[13][4] = {
        { PAL_PAGE,       2, 2, 2 },    /* light gray page          */
        { PAL_CHROME,     0, 1, 3 },    /* blue: bands and the ink  */
        { PAL_PURPLE,     2, 0, 2 },    /* Grape                    */
        { PAL_GREEN,      0, 2, 1 },    /* Sage, Basil              */
        { PAL_YELLOW,     3, 3, 0 },    /* Banana                   */
        { PAL_ORANGE,     3, 1, 0 },    /* Tangerine, Flamingo      */
        { PAL_RED,        3, 0, 0 },    /* Tomato, and the alarm    */
        { PAL_GRAPHITE,   1, 1, 1 },    /* Graphite                 */
        { 8 + FG_INK,     0, 0, 0 },
        { 8 + FG_PAPER,   3, 3, 3 },
        { 8 + FG_ACCENT,  0, 0, 2 },
        { 8 + FG_DIM,     1, 1, 1 },
    ...
    for (i = 0; i < 13; i++)
        paletteRGB(pal[i][0], pal[i][1], pal[i][2], pal[i][3]);
