 ...
 *
 * Every list row here paints columns 1 to the edge and leaves column 0
 * alone. That column is the chip gutter, and every backend keeps it out of
 * the selection bar for its own hardware reason -- the Atari because
 * inverse video draws in front of the player carrying the chip, the Apple
 * because MouseText has no inverse form. Here neither constraint exists:
 * char and attribute are independent bytes, and the rule is kept because it
 * is *right* -- the chip belongs to the event, not to the cursor, and
 * keeping it outside the bar is what lets it keep its own colour on the
 * selected row.
 *
 * The rows are laid out through the runtime columns ui_geom() chose: at 80
 * the Apple's layout with the category column, at 40 the Atari's without
 * it. w_cat == 0 is the flag for "no category column", which is also what
 * spares the 40-column screen from painting a field it has no room for.
 *
 * Colour chips: in colour mode every one of Google's eleven colour names
 * has its own ink (ink_attr in screen.c, the Adam's arrangement), painted
 * as a full-block cell -- color_chip()'s five-way quantisation is never
 * consulted at all. The two monochrome tables fall back to it, one ramp
 * glyph per chip.
 */
...
#define DETAIL_ROW      22                      /* ..23  */

/* Columns shared by the DAY and AGENDA rows; the rest are ui_geom()'s. */
#define COL_CHIP        0
#define COL_CARET       1
#define COL_MARK        2
#define COL_TIME        4
...
/* The event's colour, in the gutter: its own ink in colour mode, its ramp
...
#endif
    scr_field(row, col_title, e->title, w_title, r);
}
    ...
    for (i = 0; i < LIST_ROWS; i++) {
        ...
        if (slot & AGD_SEP) {
            /* A separator is a heading, never a selection: the chip column
               ...
               blue accent in colour and the MDA's own underline in mode 7,
 ...
 * bar one cell per event up to mo_bars. In colour the bar is full blocks in
 * the day's leading colour -- gc_daycol keeps the COL_* exactly so a
...
void ui_detail(unsigned char ev, unsigned int top)
{
     ...
     */
    rows = wrap_text(e->title, (char *) wrapbuf, 2, scr_cols, 81);
    for (i = 0; i < 2; i++) {
    ...
    for (i = 0; i < DET_WIN; i++) {
        ...
        if (top + i < gc_det_rows)
            scr_text((unsigned char) (DET_TOP + i), 1, gc_det[top + i],
                     A_TEXT);
    }
 ...
 *
 * The Atari flashes it by alternating two colour registers, which repaints
 * nothing at all. Here the flash is the row being repainted between two
 * roles -- red band and red-on-page in colour, reverse and bright on the
